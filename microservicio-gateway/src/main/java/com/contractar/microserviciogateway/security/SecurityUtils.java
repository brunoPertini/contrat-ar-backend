...
public class SecurityUtils {
	...
	public boolean hasValidClientId(HttpServletRequest request) {
        ...
        boolean isValidClient = Optional.ofNullable(clientId)
                .flatMap(cId -> Optional.ofNullable(clientSecret)
                        .map(cSecret -> this.clientId.equals(cId) && this.clientSecret.equals(cSecret)))
                .orElse(false);
